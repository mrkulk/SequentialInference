...
emission_matrix = [[0.1, 0.9] [0.6, 0.4]]
init_matrix = [0.5, 0.5]
obs_seq = [1, 1, 1, 1, 1, 1, 1, 1]
...
function initPartilce(particle, obsSeq, emission_mat, init_matrix, transition_mat)
	# currentSize = length(keys(particle))
	newObs = obsSeq[1]
	particle[1] = {"weight" => 0, "hidden_state" => 0}
	sampledState = initialize(init_matrix)
...
function samplePath(obsSeq, transition_mat, emission_mat, init_matrix, resample)
	...
	for k = 1:NUM_PARTICLES_SAMP		
		initPartilce(particles[k], obsSeq, emission_mat, init_matrix, transition_mat)
	end
