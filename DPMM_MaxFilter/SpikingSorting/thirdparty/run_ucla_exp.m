...
K = MAP_number_of_neurons;
for i=1:size(out_of_sample_training_data, 1)
    y = out_of_sample_training_data(i,:)';
    for kid=1:K %Integrating cluster assigments
        n = length(find(map_spike_sorting==kid)); 
        m_Y = PF_means(:,kid, map_spike_sorting_index,1);
        SS=PF_sum_squares(:,:,kid,map_spike_sorting_index,1);
