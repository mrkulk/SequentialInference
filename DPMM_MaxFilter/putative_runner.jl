...
function putativeResample(time, particles_t, particles_t_minus_1, log_maxfilter_probability_array, maxfilter_cid_array, maxfilter_particle_struct)
...
function run_sampler()
	...
	for time = 2:NUM_POINTS
		...
		if MAXFILTERING == 1
			...
			putativeResample(time, particles[time], deepcopy(particles[time-1]), log_maxfilter_probability_array, maxfilter_cid_array, maxfilter_particle_struct)
			#normalizeWeights(time)
			#resample(time)
			#recycle(time)
		end
